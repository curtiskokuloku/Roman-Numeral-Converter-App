...
def romanToInt(s):
    ...
    roman_dict = {'I': 1, 'V': 5, 'X': 10, 'L': 50, 'C': 100, 'D': 500, 'M': 1000}  # dictionary to store the values of each Roman numeral
    result = 0
    prev_value = 0
...
class RomanConverterApp:
    def __init__(self):
        ...
        self.label = tk.Label(self.window, text="Enter Roman Numeral:") # Creates a label widget with the given text
        self.label.pack()                               # Packs the label widget, ensuring it is displayed in the window

        self.entry = tk.Entry(self.window)              # Creates an entry widget (text box) that allows the user to input the Roman numeral
        self.entry.pack()                               # Packs the entry widget to display it in the window
        self.entry.bind("<Return>", self.on_enter)      # Bind the Enter key to the on_enter method
    ...
    def convert(self):
        ...
        roman_numeral = self.entry.get().upper()        # Retrieves the input from the entry widget, converts it to uppercase
        
        if not roman_numeral:
            messagebox.showerror("Error", "Please enter a Roman numeral.")  # Display error message for empty input
            return

        try:    # Handle exceptions
            numeric_value = romanToInt(roman_numeral)
            messagebox.showinfo("Result", f"{roman_numeral} is equal to {numeric_value}")
